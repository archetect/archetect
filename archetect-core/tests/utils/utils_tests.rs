...
use archetect_api::{sync_io_channel, ClientIoHandle, ScriptMessage};
...
fn test_utils() -> Result<(), ArchetectError> {
    ...
    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::LogTrace(message) => {
        assert_eq!(message, "Trace Level");
    });

    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::LogDebug(message) => {
        assert_eq!(message, "Debug Level");
    });

    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::LogInfo(message) => {
        assert_eq!(message, "Info Level");
    });

    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::LogWarn(message) => {
        assert_eq!(message, "Warn Level");
    });

    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::LogError(message) => {
        assert_eq!(message, "Error Level");
    });

    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::Display(message) => {
    ...
    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::Print(message) => {
    ...
    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::Print(uuid) => {
        use std::str::FromStr;
...
fn test_switches() -> Result<(), ArchetectError> {
    ...
    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::Print(build_switch_enabled) => {
        assert_eq!(build_switch_enabled, "true");
    });

    assert_matches!(client_handle.receive().unwrap(), ScriptMessage::Print(test_switch_enabled) => {
