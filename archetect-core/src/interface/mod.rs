...
/// Compare a DECLARED interface (`interface:` / `interface.yaml`) against
/// the DERIVED one. Returns human-readable drift findings; empty means
/// the declaration tells no lies the probe could catch. Part of the
/// declarative interface's retirement: first make drift visible, then
/// delete the file.
pub fn check_drift(
    declared: &crate::archetype::archetype_manifest::interface::ArchetypeInterface,
    derived: &DerivedInterface,
) -> Vec<String> {
    use crate::archetype::archetype_manifest::interface::PromptType as DeclaredType;

    let mut findings = Vec::new();

    let derived_keys: BTreeMap<&str, &InterfacePrompt> = derived
        .prompts
        .iter()
        .filter_map(|p| p.envelope.key.as_deref().map(|k| (k, p)))
        .collect();

    for declared_prompt in &declared.prompts {
        match derived_keys.get(declared_prompt.key.as_str()) {
            None => findings.push(format!(
                "declared prompt `{}` was never asked by the script (phantom declaration)",
                declared_prompt.key
            )),
            Some(actual) => {
                let matches = matches!(
                    (&declared_prompt.prompt_type, &actual.envelope.prompt_type),
                    (DeclaredType::Text, PromptType::Text)
                        | (DeclaredType::Int, PromptType::Int)
                        | (DeclaredType::Bool, PromptType::Bool)
                        | (DeclaredType::Select, PromptType::Select)
                        | (DeclaredType::Multiselect, PromptType::MultiSelect)
                        | (DeclaredType::List, PromptType::List)
                        | (DeclaredType::Editor, PromptType::Editor)
                );
                if !matches {
                    findings.push(format!(
                        "declared prompt `{}` has type {:?} but the script asks {:?}",
                        declared_prompt.key, declared_prompt.prompt_type, actual.envelope.prompt_type
                    ));
                }
            }
        }
    ...
    let declared_keys: std::collections::HashSet<&str> =
        declared.prompts.iter().map(|p| p.key.as_str()).collect();
    for prompt in &derived.prompts {
        if let Some(key) = prompt.envelope.key.as_deref() {
            if !declared_keys.contains(key) {
                findings.push(format!(
                    "script prompts for `{}` but the declared interface omits it",
                    key
                ));
            }
        }
    ...
    let declared_switches: std::collections::HashSet<&str> =
        declared.switches.iter().map(|s| s.key.as_str()).collect();
    for switch in &derived.switches {
        if !declared_switches.contains(switch.as_str()) {
            findings.push(format!(
                "script consults switch `{}` but the declared interface omits it",
                switch
            ));
        }
    }
