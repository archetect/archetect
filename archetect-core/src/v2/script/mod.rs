...
pub (crate) fn create_environment(runtime_context: RuntimeContext) -> Environment<'static> {
    ...
    environment.add_filter("constant_case", |value: Cow<'_, str>| cruet::to_screaming_snake_case(value.as_ref()));
    environment.add_filter("directory_case", |value: Cow<'_, str>| to_directory_case(value.as_ref()));
    ...
    environment.add_filter("pascal_case", |value: Cow<'_, str>| cruet::to_pascal_case(value.as_ref()));
    environment.add_filter("package_case", |value: Cow<'_, str>| to_package_case(value.as_ref()));
    environment.add_filter("sentence_case", |value: Cow<'_, str>| cruet::to_sentence_case(value.as_ref()));
