...
prova.test("an INTERACTIVE archetype's page is answered in one exchange", {
	promises = "HELD — the need is not demonstrated. This only matters for archetypes "
		.. "classified `interactive`; a `batch` one is collected up front from `describe` "
		.. "and rendered with zero prompt exchanges, which the two proofs above already "
		.. "show end to end.\n\n"
		.. "What is known about the production catalog (p6m-archetypes3, 2026-08-18, 81 "
		.. "archetypes): 80 probe cleanly on the default path — the one exception is the "
		.. "catalog entry itself, which is not an archetype. Forms are short-to-moderate: "
		.. "median 2 prompts, mean 4.3, max 13, with 24 above eight. None declare pages "
		.. "yet, and a page wrapping a shared-library prompt call (the composition shape "
		.. "this catalog is built on) captures that library's prompts correctly.\n\n"
		.. "What is NOT known: how many classify `batch` under `--explore`. That survey "
		.. "exceeded its time box and was stopped, because by then it no longer decided "
		.. "anything. Run it before building this — if most of the catalog is `batch`, "
		.. "the population this feature serves is small and the existing zero-exchange "
		.. "path already covers it.\n\n"
		.. "If it is needed: the look-ahead comes from the PROBE, not from re-running the "
		.. "page. When the script blocks on a prompt, the server knows from the probe's "
		.. "layout which page it sits in and sends that page's remaining envelopes as one "
		.. "batch; the client answers them together; the server feeds the first answer to "
		.. "the blocked prompt and seeds the rest into the context, so the later prompts "
		.. "resolve from answers and never reach the wire. The page body executes exactly "
		.. "once. An earlier sketch ran the body twice — discovery, then real — which "
		.. "would render a `catalog.render` child twice; rejected, because an authoring "
		.. "surface where adding a page silently doubles what a script does is a footgun "
		.. "documentation cannot fix. Needs a ClientMessage variant for the answer map, "
		.. "the prompt handler absorbing surplus keys, a per-session probe cache, and a "
		.. "fallback to asking individually when the script diverges from the probe.",
}, function(t)
	local srv = t:use(server)
