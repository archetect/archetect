...
prova.test("an INTERACTIVE archetype's page is answered in one exchange", {
	promises = "the original premise — 'a wizard's Next costs N round trips' — turned out "
		...
		.. "The look-ahead comes from the PROBE, not from running the page twice. When the "
		.. "script blocks on a prompt, the server knows from the probe's layout which page "
		.. "that prompt sits in, and sends the page's remaining envelopes as one batch. "
		.. "The client answers them together; the server hands the first answer to the "
		.. "blocked prompt and seeds the rest into the context, so the later `prompt_*` "
		.. "calls in that page resolve from answers and never reach the wire. The page "
		.. "body executes EXACTLY ONCE.\n\n"
		.. "An earlier sketch had the engine run the body twice — once to discover the "
		.. "prompts, once for real with the answers seeded — which would have run a page's "
		.. "side effects twice. A `catalog.render` inside a page would render its child "
		.. "twice; suppressing writes and exec on the discovery pass does not cover it. "
		.. "Rejected: an authoring surface where adding a page silently doubles what a "
		.. "script does is a footgun no amount of documentation pays for. Probing already "
		.. "happens in a sandbox that discards writes and forbids exec, so reusing it "
		.. "moves no hazard into the render path.\n\n"
		.. "Needs: a `ClientMessage` variant carrying a keyed answer map, the prompt "
		.. "handler absorbing the surplus keys into the context, a probe result cached "
		.. "per session, and a fallback to asking individually when the script diverges "
		.. "from the probe. Plus a proof that counts exchanges rather than asserting an "
		.. "exit code.",
}, function(t)
	local srv = t:use(server)
