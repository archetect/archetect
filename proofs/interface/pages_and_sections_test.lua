...
prova.test("an INTERACTIVE archetype's page is answered in one exchange", {
	promises = "HELD DELIBERATELY — the need is not demonstrated. This only matters for "
		.. "archetypes classified `interactive`; a `batch` one is collected up front from "
		.. "`describe` and rendered with zero prompt exchanges, which the two proofs above "
		.. "already show end to end.\n\n"
		.. "Measured against the production catalog (2026-08-18, 73 v3 ports): 70 "
		.. "classified `interactive` — and every one of them for the WRONG reason. They "
		.. "abort on the first probe run against globals removed in the v3 refinement "
		.. "(`switches.` → `archetype.switches.`, and `component.`), not because their "
		.. "prompt sets are dynamic. Patching the `switches` call in five of them flipped "
		.. "three straight to `batch` with a clean probe; the other two hit `component`. "
		.. "Of the archetypes that actually run today, ALL classify `batch`. So the "
		.. "population this feature would serve is currently zero, and the honest next "
		.. "step is to port the catalog and re-measure — not to build a protocol change "
		.. "for a need nobody has shown.\n\n"
