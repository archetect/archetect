...
use crate::{autocompletion::{Autocomplete, NoAutoCompletion, Replacement}, config::{self, get_configuration}, error::{InquireError, InquireResult}, input::Input, terminal::get_default_terminal, ui::{Backend, Key, KeyModifiers, ListBackend, RenderConfig}, validator::{ErrorMessage, StringValidator, Validation}};
use crate::formatter::ListFormatter;
use crate::validator::ListValidator;
...
pub struct List<'a> {
    ...
    ///
    /// Item Validators are executed in the order they are stored, stopping at and displaying to the user
    /// only the first validation error that might appear.
    ///
    /// The possible error is displayed to the user one line above the prompt.
    pub item_validators: Vec<Box<dyn StringValidator>>,

    /// Collection of validators to apply to the entire List.
    ///
    /// List Validators are executed in the order they are stored, stopping at and displaying to the user
    /// only the first validation error that might appear.
    ///
    /// List Validators are applied after Item Validators, and only when an attempt is made to complete
    /// the List.
    ///
    /// The possible error is displayed to the user one line above the prompt.
    pub list_validators: Vec<Box<dyn ListValidator>>,
...
impl<'a> List<'a> {
    ...
    pub const DEFAULT_FORMATTER: ListFormatter<'a> = &|list: &Vec<String> | {
        list.iter()
    ...
    /// Default validators added to the [List] prompt for validating each list entry
    pub const DEFAULT_ITEM_VALIDATORS: Vec<Box<dyn StringValidator>> = vec![];

    /// Default validators added to the [List] prompt for validating the entire lists
    pub const DEFAULT_LIST_VALIDATORS: Vec<Box<dyn ListValidator>> = vec![];

    /// Default help message.
    pub const DEFAULT_HELP_MESSAGE: Option<&'static str> =
        Some("Empty <return> when finished");
    ...
    pub fn with_initial_value(mut self, message: &str) -> Self {
        self.initial_value = Some(message.into());
        self
    }
    ...
    /// Sets the formatter.
    pub fn with_formatter(mut self, formatter: ListFormatter<'a>) -> Self {
    ...
    ///
    /// The possible error is displayed to the user one line above the prompt.
    pub fn with_item_validator<V>(mut self, validator: V) -> Self
    where
        V: StringValidator + 'static,
    {
        ...
        if self.item_validators.capacity() == 0 {
            self.item_validators.reserve(5);
        }

        self.item_validators.push(Box::new(validator));
        self
    }
    ...
    ///
    /// The possible error is displayed to the user one line above the prompt.
    pub fn with_list_validator<V>(mut self, validator: V) -> Self
        where
            V: ListValidator + 'static,
    {
        // Directly make space for at least 5 elements, so we won't to re-allocate too often when
        // calling this function repeatedly.
        if self.list_validators.capacity() == 0 {
            self.list_validators.reserve(5);
        }

        self.list_validators.push(Box::new(validator));
        self
    }
    ...
    ///
    /// The possible error is displayed to the user one line above the prompt.
    pub fn with_item_validators(mut self, validators: &[Box<dyn StringValidator>]) -> Self {
        for validator in validators {
            #[allow(suspicious_double_ref_op)]
            self.item_validators.push(validator.clone());
        }
        self
    }
    ...
    /// Adds the validators to the collection of validators in the order they are given.
    /// You might want to use this feature in case you need to require certain features
    /// from the user's answer, such as defining the minimum or maximum number of items.
    ///
    /// Validators are executed in the order they are stored, stopping at and displaying to the user
    /// only the first validation error that might appear.
    ///
    /// The possible error is displayed to the user one line above the prompt.
    pub fn with_list_validators(mut self, validators: &[Box<dyn ListValidator>]) -> Self {
        for validator in validators {
            #[allow(suspicious_double_ref_op)]
            self.list_validators.push(validator.clone());
        }
        self
    }
    ...
    pub(crate) fn prompt_with_backend<B: ListBackend>(self, backend: &mut B) -> InquireResult<Vec<String>> {
        ListPrompt::from(self).prompt(backend)
    }
}
...
struct ListPrompt<'a> {
    ...
    input: Input,
    formatter: ListFormatter<'a>,
    item_validators: Vec<Box<dyn StringValidator>>,
    list_validators: Vec<Box<dyn ListValidator>>,
...
impl<'a> From<List<'a>> for ListPrompt<'a> {
    fn from(so: List<'a>) -> Self {
        ...
        Self {
            ...
            item_validators: so.item_validators,
            list_validators: so.list_validators,
        }
    }
}
...
impl<'a> ListPrompt<'a> {
    pub fn get_answers(&mut self) -> Vec<String> {
    ...
    fn get_current_answer(&self) -> String {
    ...
    fn validate_item(&self) -> InquireResult<Validation> {
        for validator in &self.item_validators {
            match validator.validate(self.get_current_answer().as_str()) {
                Ok(Validation::Valid) => {}
    ...
    fn validate_list(&self) -> InquireResult<Validation> {
        for validator in &self.list_validators {
            match validator.validate(&self.options) {
                Ok(Validation::Valid) => {}
                Ok(Validation::Invalid(msg)) => return Ok(Validation::Invalid(msg)),
                Err(err) => return Err(InquireError::Custom(err)),
            }
    ...
    fn render<B: ListBackend>(&mut self, backend: &mut B) -> InquireResult<()> {
        ...
        backend.render_prompt(prompt, &self.options, self.default.as_ref())?;

        backend.render_options(&self.options, &self.input)?;
    ...
    fn prompt<B: ListBackend>(mut self, backend: &mut B) -> InquireResult<Vec<String>> {
        self.update_suggestions()?;
        ...
        loop {
            ...
            match key {
                ...
                Key::Submit => match self.validate_item()? {
                    Validation::Valid => {
                        let input = self.input.content().trim();
                        if input.is_empty() {
                            match self.validate_list()? {
                                Validation::Valid => {
                                    break;
                                }
                            ...
                            self.options.push(self.input.content().to_string());
                            self.input.clear();
                        }
                    }
                    Validation::Invalid(msg) => self.error = Some(msg),
        ...
        let formatted = (self.formatter)(&self.get_answers());

        finish_prompt_with_answer!(backend, &self.message, &formatted, self.get_answers());
    }
}
...
mod test {
    ...
    //     },
    //     "12345yes",
    //     List::new("").with_item_validator(|ans: &str| match ans.len() {
