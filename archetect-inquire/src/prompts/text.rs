...
impl<'a> Text<'a> {
    ...
    pub const DEFAULT_HELP_MESSAGE: Option<String> = None;
    ...
    pub fn with_default(mut self, message: &str) -> Self {
        self.default = Some(message.into());
        self
    }
...
impl<'a> TextPrompt<'a> {
    ...
    fn get_current_answer(&self) -> String {
        ...
        if self.input.content().is_empty() {
            if let Some(val) = self.default.clone() {
                return val;
            }
        }
    ...
    fn render<B: TextBackend>(&mut self, backend: &mut B) -> InquireResult<()> {
        ...
        backend.render_prompt(prompt, self.default.as_ref(), &self.input)?;
